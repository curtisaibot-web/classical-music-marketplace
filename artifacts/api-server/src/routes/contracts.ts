...
function generateHtmlPdf(contract: typeof contractsTable.$inferSelect, teacherName: string): string {
  ...
  body = body.replace(/\{\{[^}]+\}\}/g, (m) => m.replace(/{{|}}/g, "").split(/(?=[A-Z])/).join(" ").toLowerCase());
...
<div class="meta">
  <p><strong>Prepared by:</strong> ${teacherName}</p>
...
<div class="signature-block">
  <div class="sig-row">
    <div class="sig-col">
      <div class="sig-line">${contract.status === "signed" ? contract.signerName ?? "" : ""}</div>
      <div class="sig-label">Client Signature — ${contract.clientName ?? ""}</div>
    </div>
    <div class="sig-col">
  ...
  ${contract.status === "signed" ? `<div class="signed-badge">✓ Signed by ${contract.signerName} on ${contract.signedAt ? new Date(contract.signedAt).toLocaleDateString() : ""}</div>` : ""}
</div>
...
router.post("/contracts/:id/send", requireAuth, async (req, res): Promise<void> => {
  ...
  const baseUrl = process.env.PUBLIC_APP_URL ?? "https://harmonia.app";
...
router.get("/contracts/:id/pdf", requireAuth, async (req, res): Promise<void> => {
  ...
  const html = generateHtmlPdf(contract, "Teacher");
...
router.get("/contracts/sign/:token", async (req, res): Promise<void> => {
  ...
  if (contract.status === "signed") {
    ...
    </head><body><h1>Contract Already Signed</h1><p>This contract was signed by <strong>${contract.signerName}</strong> on ${contract.signedAt ? new Date(contract.signedAt).toLocaleDateString() : ""}.</p></body></html>`;
    res.setHeader("Content-Type", "text/html");
    res.send(html);
    return;
  }
...
<div class="sign-form">
  ...
  <form method="POST" action="/api/contracts/sign/${token}">
    <div>
      <label for="signerName">Full Name</label>
      <input type="text" id="signerName" name="signerName" required placeholder="Enter your full legal name" />
    </div>
    <label>
...
router.post("/contracts/sign/:token", async (req, res): Promise<void> => {
  ...
  if (!signerName?.trim()) {
    res.status(400).send("<h1>Error: Full name is required.</h1>");
    return;
  }
  ...
  if (contract.status === "signed") {
    res.send(`<h1>Already signed by ${contract.signerName}</h1>`);
    return;
  }
...
<p>Signed by <strong>${signerName}</strong> on ${new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}.</p>
<p>A copy has been sent to your teacher. Thank you!</p>
