...
function generateInvoiceHtml(
  ...
  const lineItemsHtml = lineItems.length > 0
    ? lineItems.map(li => `
      <tr>
        <td style="padding:10px;border-bottom:1px solid #e5e5d0;">${escapeHtml(li.description)}</td>
...
<div class="header">
  <div>
    ...
    <p style="margin:4px 0;color:#666;font-size:13px;">${escapeHtml(teacherName)}</p>
  </div>
  <div class="invoice-number">
    ...
    <span class="status-badge">${escapeHtml(invoice.status.toUpperCase())}</span>
  </div>
</div>
...
<div class="bill-section">
  <div class="bill-from">
    <h4>From</h4>
    <p><strong>${escapeHtml(teacherName)}</strong></p>
  </div>
  <div class="bill-to">
    <h4>Bill To</h4>
...
${invoice.paymentNote ? `<div class="notes"><strong>Payment Instructions:</strong><br>${escapeHtml(invoice.paymentNote)}</div>` : ""}
${invoice.notes ? `<div class="notes" style="margin-top:10px;"><strong>Notes:</strong><br>${escapeHtml(invoice.notes)}</div>` : ""}
